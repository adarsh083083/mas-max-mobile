...
const AddVehicles = () => {
  ...
  const handleFormSubmit = () => {
    ...
    if (!hasError && vehicleInfo.model_id) {
      dispatch(
        getVehicleThunk.addVehicleAsyncThunk(vehicleInfo),
        (vehicleInfo.is_profile_completed = true),
      )
        .unwrap()
        .then(res => {
          if (res) {
            ...
            if (checkPrevPage?.serviceDetailsPage) {
        ...
        .catch(error => {
          Toast.show({
            type: 'error',
            text1: error?.error?.registration_number,
          });
        });
    } else {
      return;
